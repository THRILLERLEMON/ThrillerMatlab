...
Soiltp = 'E:\OFFICE\LAIO_data_JQL\HWSD_LP_WGS1984_90m\HWSD_LP_90m_masked.tif';  % 土壤类型空间数据
Soil_vd = [11020,11934];  % 土壤类型数据有效值范围

Alpha = 'E:\OFFICE\LAIO_data_JQL\AlpLmd_ave_90mmasked\LP_Alpha_00to12_9090masked.tif';  % α数据
Alpha_vd = [0,100];  % α数据有效范围

Lamda = 'E:\OFFICE\LAIO_data_JQL\AlpLmd_ave_90mmasked\LP_Lamda_00to12_9090masked.tif';  % λ数据
Lamda_vd = [0,1];  % λ数据有效范围

Vegm = 'E:\OFFICE\LAIO_data_JQL\Landcover2010_90m_masked\landcover2010_90m_masked.tif';  %植被类型空间数据
Vegm_vd = [101,607];  % λ数据有效范围

DemSRTM = 'E:\OFFICE\LAIO_data_JQL\srtm_lp_wgs1984_90m\srtm_lp_90m_masked.tif';  %DEM数据
DemSRTM_vd = [87,5206];  %DEM数据变化梯度

bv = -9999;  % 输出结果背景值
clsgrid_out = 'E:\OFFICE\LAIO_data_JQL\Zone\HG_GridCls2010.txt';  % 输出分类格点编号空间结果
clslist_out = 'E:\OFFICE\LAIO_data_JQL\Zone\HG_Clslist2010.txt';  % 分类组合列表
