...
outpt = '/home/test2/MTE_NEE/JungNEE/Stat/Gra_MTE_sts02';
...
for ir = 1:360
    for ic = 1:720
        ...
        if any(~isnan(tmp(:)))
            ...
            gtmp = atmp;
            if (ir>=150 && ir<=300) && (ic>=180 && ic<=300)
                % 10.Grasslands 7.Open shrublands 9.Savannas
                gtmp(~(tmp==10|tmp==7|tmp==9)) = nan;
            else
                gtmp(~(tmp==10|tmp==7)) = nan;
            end
